        ...
        try:
            ...
            for i, brand_name in enumerate(self.brand_names):
                ...
                if qogita_products:
                    ...
                    # Match with Amazon data
                    self.log_message.emit(f"Matching {len(qogita_products)} products with Amazon...")
                    matched_products = self.product_matcher.match_products_bulk(qogita_products)
    ...
    def _scan_brand(self, brand_name: str) -> List[QogitaProduct]:
        ...
        try:
            ...
            for product_data in raw_products:
                try:
                    # Parse the CSV data into QogitaProduct
                    qogita_product = QogitaProduct(
                        gtin=product_data.get('GTIN', ''),
                        name=product_data.get('Name', ''),
                        category=product_data.get('Category', ''),
                        brand=product_data.get('Brand', ''),
                        wholesale_price=float(product_data.get('€ Lowest Price inc. shipping', 0)),
...
class ProductTableWidget(QTableWidget):
    ...
    def setup_table(self):
        ...
        header.setSectionResizeMode(8, QHeaderView.ResizeMode.ResizeToContents)  # Stock
        header.setSectionResizeMode(9, QHeaderView.ResizeMode.ResizeToContents)  # Status
        header.setSectionResizeMode(10, QHeaderView.ResizeMode.ResizeToContents) # ASIN
    ...
    def add_matched_product(self, matched_product: MatchedProduct):
        ...
        self.setItem(row, 8, QTableWidgetItem(str(qp.stock)))
        ...
        self.setItem(row, 10, QTableWidgetItem(matched_product.amazon_asin or "N/A"))
...
class QogitaScannerWindow(QWidget):
    ...
    def __init__(self):
        ...
        # Initialize APIs
        self.qogita_api = QogitaAPI(self.config)
    ...
    def add_matched_product(self, matched_product: MatchedProduct):
        """Add a matched product to the results"""
